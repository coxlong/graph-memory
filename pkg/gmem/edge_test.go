...
func TestUpsertEdgeExpiredAt(t *testing.T) {
	...
	res, _ := c.SearchEdges("member of ExpTeam", 10, false)
	if len(res) != 0 {
		t.Fatalf("expired edge should be hidden by default: %v", res)
	}
	// as-of before the expiry: edge active
	res, err = c.searchEdgesFiltered("member of ExpTeam", 10, "2026-05-01T00:00:00Z", false)
	if err != nil {
		t.Fatal(err)
	}
	if len(res) != 1 {
	...
	res, _ = c.SearchEdges("member of ExpTeam", 10, true)
	if len(res) != 1 {
